  ...
  return (
    <SafeAreaView className={`flex-1 ${bgColor}`}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} className="px-4">
        <Text className={`text-lg font-bold mt-4 ${textColor}`}>{selectedDateObj.monthName} {selectedDateObj.year}</Text>
          ...
          {next30Days.map((item, index) => {
            ...
            return (
              <TouchableOpacity
                ...
                className={`items-center justify-center w-16 h-20 mr-3 rounded-2xl ${isSelected ? 'bg-[#D81B8C]' : cardBgColor}`}
              >
                <Text className={`text-lg font-bold ${isSelected ? 'text-white' : textColor}`}>{item.dateNum}</Text>
                <Text className={`text-xs uppercase ${isSelected ? 'text-white' : subTextColor}`}>{item.dayName}</Text>
              </TouchableOpacity>
            );
          })}
        </ScrollView>

        <Text className={`text-lg font-bold mt-4 mb-4 ${textColor}`}>Choose Wedding Stylist</Text>
          ...
          {STYLISTS.map((stylist) => {
            ...
            return (
              <TouchableOpacity key={stylist.id} onPress={() => setSelectedStylist(stylist.name)} className="mr-5 items-center">
                ...
                <Text className={`mt-2 font-semibold ${isSelected ? 'text-[#D81B8C]' : isDarkMode ? 'text-white' : 'text-black'}`}>{stylist.name}</Text>
        ...
        {Object.entries(TIME_SLOTS).map(([period, slots]) => (
          <View key={period} className="mb-4">
            <Text className={`font-semibold mb-3 ${subTextColor}`}>{period}</Text>
              ...
              {slots.map((time) => {
                ...
                return (
                  <TouchableOpacity
                    ...
                    className={`px-2 py-3 rounded-xl w-[30%] mr-[3%] mb-3 items-center border ${!available ? (isDarkMode ? 'bg-gray-900 border-gray-800' : 'bg-gray-50 border-gray-100') : isSelected ? 'bg-[#D81B8C] border-[#D81B8C]' : (isDarkMode ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-200')
                      }`}
                  >
                    <Text className={`text-[11px] font-bold ${!available ? 'text-gray-300' : isSelected ? 'text-white' : (isDarkMode ? 'text-white' : 'text-black')}`}>{time}</Text>
                  </TouchableOpacity>
                );
              })}
            </View>
      ...
      <View className={`absolute bottom-6 w-full px-4 pb-2 ${isDarkMode ? 'bg-[#121212]/80' : 'bg-white/80'}`}>
        <TouchableOpacity
